...
export const projects: Project[] = [
  {
    ...
    narrative: [
      ...
      },
      {
        ...
        body: "The bottleneck was the data, not the architecture. Blast fishing is illegal and rare — which is why you want a detector, and why you cannot go record more positives. ReefSet v1.0 gave us 57,074 hand-labeled hydrophone clips across 16 sites, and only 203 of those were confirmed Indonesia blasts (`anthrop_bomb`). A 0.36% positive class. Train naively and the model learns the profitable lie: never fire. A conservation tool that never fires is a brick. The second data problem showed up the moment we left the dataset. The 38 held-out field recordings (\"Drive\") came from a different hydrophone, different gain, different site. Ranking stayed strong — ROC-AUC ~0.95–0.98, the model still ordered a blast above a shrimp — but the confidence threshold calibrated on ReefSet did not travel. Field recall started at 7/38 (18%). Opening the negative class to a balanced slice of all 37 ReefSet label types moved that to 12/38 (32%). That's the gap still open: not whether a spectrogram contains a bomb, but whether this recorder, at this site, at this gain, agrees with the last one.",
      },
    ],
    numbers: {
  ...
  },
  {
    ...
    narrative: [
      ...
      },
      {
        ...
        wide: true,
        body: [
          "The suggestion has to look at your calendar, your location, and whether Focus is on — which is exactly the data you do not hand to a server. So the intelligence layer never leaves the phone.",
          "`ContextBuilder` pulls what's already local: EventKit as a three-hour free/busy window (busy now, free until, next gap — not the event titles), Core Location, motion, weather, a Focus social-gate, and friend presence that arrived as ciphertext and was decrypted here. `OpportunityRanker` scores that with cheap signals — both free, walking distance, shared interest, time of day, past hangouts — and hard-gates the rest: Focus on means silence; driving means silence; Eclipse stays hidden. Only the top three candidates ever reach Apple's on-device Foundation Models (`SystemLanguageModel` / `LanguageModelSession`). The model is a reconnection coach, not a chatbot: it is not allowed to invent friends or venues, it returns strongly-typed `@Generable` output (`PulseSuggestionBundle`), and a MapKit tool (`FindNearbyVenueTool`) grounds \"coffee nearby\" in a real place. If Apple Intelligence isn't on the device yet, `FallbackComposer` still fills the map from the ranked facts so the product never looks broken.",
          "That split is why the backend can be dumb. Friend-to-friend location is sealed with **CryptoKit** (`CryptoBox`: X25519 key agreement, ChaCha20-Poly1305) before it hits the Spring Boot API; the server only stores and relays `EncryptedBlob` ciphertext it cannot read. A Swift `actor` `APIClient` over `URLSession` talks JSON REST — Bearer auth, token refresh, retries when the box is cold — because the server is a relay, not a place that thinks. Tokens and identity keys stay in the **Keychain**. Pairing is opt-in (invite codes / QR, Apple-only sign-in), so there is no public graph to scrape. Siri (`HowAreMyFriendsDoingIntent`, pulse actions), WidgetKit, and Live Activities are just other surfaces for the same on-device answer. The UI is SwiftUI with feature stores (`SuggestionStore`) because the suggestion is a live object, not a screen with a view model; `KismetTests` covers the ranker and the crypto box.",
        ],
      },
      {
        heading: "My contribution",
